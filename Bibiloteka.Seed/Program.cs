            ...
            using (ExcelPackage package = new ExcelPackage(file))
            {
                using (UnitOfWork unit = new UnitOfWork())
                {
                  ...
                  //  unit.Context.Database.EnsureCreated();  command dont work on this version,explore more
                    ...
                    Console.WriteLine("Publishers seeded, press any key to continue loading books");
                    Console.ReadKey();
                    Knjige.Collect(package.Workbook.Worksheets["Books"], unit);
                    Console.WriteLine("Books seeded, press any key to continue loading authors");
                    Console.ReadKey();
                    Autori.Collect(package.Workbook.Worksheets["Authors"], unit);
                    Console.WriteLine("Authors seeded, press any key to continue loading autorbooks");
                    Console.ReadKey();
